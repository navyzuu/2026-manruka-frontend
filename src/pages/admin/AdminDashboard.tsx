...
interface Room { id: number; name: string; capacity: number; description?: string; }
            ...
            <main className="w-[95%] max-w-[2400px] mx-auto mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                     ...
                     <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-5">
                        ...
                        <div><p className="text-slate-500 text-sm font-bold uppercase">Menunggu</p><h3 className="text-4xl font-extrabold text-slate-800">{stats.totalPending}</h3></div>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-5">
                        <div className="p-4 bg-green-100 text-green-600 rounded-xl">
                            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                        <div><p className="text-slate-500 text-sm font-bold uppercase">Hari Ini</p><h3 className="text-4xl font-extrabold text-slate-800">{stats.todayApproved}</h3></div>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-5">
                        ...
                        <div><p className="text-slate-500 text-sm font-bold uppercase">Ditolak</p><h3 className="text-4xl font-extrabold text-slate-800">{stats.totalRejected}</h3></div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left table-fixed">
                            <thead className="bg-slate-50 text-slate-500 border-b uppercase tracking-wider text-xs">
                                <tr>
                                    <th className="px-8 py-4 w-[25%]">Peminjam</th>
                                    <th className="px-8 py-4 w-[30%]">Ruangan & Waktu</th>
                                    <th className="px-8 py-4 w-[20%]">Status</th>
                                    {/* FIX: Width Tetap pada kolom Aksi */}
                                    <th className="px-8 py-4 text-center w-[200px]">Aksi</th>
                                </tr>
                            </thead>
                                ...
                                {filterData(currentTab === 'active' ? activeBookings : currentTab === 'history' ? historyBookings : rejectedBookings).map(b => (
                                    <tr key={b.id} className={`hover:bg-slate-50 transition-colors ${currentTab !== 'active' ? 'opacity-80' : ''}`}>
                                        ...
                                        <td className="px-8 py-5 text-center">
                                            {/* FIX: Wrapper dengan tinggi tetap agar tidak loncat */}
